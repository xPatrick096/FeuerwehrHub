...
export async function renderMyArea() {
  ...
  content.innerHTML = `
    ...
    <div class="tab-bar" style="display:flex;gap:4px;margin-bottom:24px;border-bottom:1px solid #e2e8f0;padding-bottom:0">
      <button class="tab-btn active" data-tab="profile"        style="padding:8px 16px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;cursor:pointer;font-size:13px;font-weight:600;margin-bottom:-1px">${icon('user', 14)} Mein Profil</button>
      <button class="tab-btn"        data-tab="qualifications" style="padding:8px 16px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;cursor:pointer;font-size:13px;font-weight:600;margin-bottom:-1px">${icon('graduation-cap', 14)} Qualifikationen</button>
      <button class="tab-btn"        data-tab="honors"         style="padding:8px 16px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;cursor:pointer;font-size:13px;font-weight:600;margin-bottom:-1px">${icon('award', 14)} Ehrungen</button>
      <button class="tab-btn"        data-tab="equipment"      style="padding:8px 16px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;cursor:pointer;font-size:13px;font-weight:600;margin-bottom:-1px">${icon('wrench', 14)} Ausrüstung</button>
      <button class="tab-btn"        data-tab="appointments"   style="padding:8px 16px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;cursor:pointer;font-size:13px;font-weight:600;margin-bottom:-1px">${icon('calendar', 14)} Termine</button>
    </div>
    ...
    wrap.innerHTML = `
      ...
      <div class="card" style="max-width:560px">
        ...
        <div class="card__body">
          ...
          <p style="font-size:12px;color:#64748b;margin-top:8px">Benutzername und Anzeigename können unter <a href="#/settings" style="color:#e63022">Einstellungen</a> geändert werden.</p>
        </div>
      </div>
...
async function renderEmergencyContacts() {
  ...
  try {
    ...
    if (!contacts.length) {
      listEl.innerHTML = '<p style="color:#64748b;font-size:13px;margin-bottom:8px">Noch keine Notfallkontakte hinterlegt.</p>';
      return;
    }
    ...
    listEl.innerHTML = contacts.map(c => `
      <div class="ec-row" data-id="${c.id}" style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;margin-bottom:8px;align-items:center">
        <input type="text" class="ec-name" value="${esc(c.name)}" placeholder="Name" maxlength="100"
          style="background:#f4f5f7;border:1px solid #e2e8f0;color:#1e293b;padding:7px 10px;border-radius:6px;font-size:13px" />
  ...
  try {
    ...
    if (!qualifications.length) {
      wrap.innerHTML = `
        <div class="card">
          <div class="card__body">
            <p style="color:#64748b;font-size:13px">Noch keine Qualifikationen hinterlegt. Der Wehrleiter kann diese im Personal-Modul eintragen.</p>
          </div>
        </div>`;
      return;
    }
    ...
    const rows = qualifications.map(q => {
      ...
      return `
        <tr>
          ...
          <td>${q.acquired_at ? formatDate(q.acquired_at) : '<span style="color:#64748b">–</span>'}</td>
          <td>${expiryText}</td>
          <td>${q.notes ? esc(q.notes) : '<span style="color:#64748b">–</span>'}</td>
        </tr>`;
    }).join('');
          ...
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead>
              <tr style="background:#f4f5f7;text-align:left">
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Qualifikation</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Erworben</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Gültig bis</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Hinweis</th>
              </tr>
            </thead>
            <tbody>
              ${rows}
  ...
  try {
    ...
    if (!honors.length) {
      wrap.innerHTML = `
        <div class="card">
          <div class="card__body">
            <p style="color:#64748b;font-size:13px">Noch keine Ehrungen hinterlegt. Ehrungen werden vom Wehrleiter im Personal-Modul eingetragen.</p>
          </div>
        </div>`;
      renderIcons(wrap);
      return;
    ...
    const rows = honors.map(h => {
      ...
      return `
        <tr style="border-bottom:1px solid #e2e8f0">
          ...
          <td style="padding:10px 16px;color:#64748b">${h.notes ? esc(h.notes) : '–'}</td>
        </tr>`;
    }).join('');
      ...
      <div class="card">
        ...
        <div class="card__body" style="padding:0">
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead>
              <tr style="background:#f4f5f7;text-align:left">
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Ehrung</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Verliehen am</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Status</th>
                <th style="padding:10px 16px;color:#64748b;font-weight:600;border-bottom:1px solid #e2e8f0">Notizen</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
  ...
  try {
    ...
    if (!equipment.length) {
      wrap.innerHTML = `
        <div class="card">
          <div class="card__body">
            <p style="color:#64748b;font-size:13px">Noch keine Ausrüstung / Ausweise hinterlegt. Diese werden vom Gerätewart oder Wehrleiter eingetragen.</p>
          </div>
        </div>`;
      return;
    }
    ...
    const cards = equipment.map(e => `
      <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:16px">
        ...
        ${e.identifier ? `<div style="font-size:13px;color:#1e293b;margin-bottom:4px">Nr./Bezeichnung: <strong>${esc(e.identifier)}</strong></div>` : ''}
        ${e.issued_at  ? `<div style="font-size:12px;color:#64748b">Ausgestellt: ${formatDate(e.issued_at)}</div>` : ''}
        ${e.expires_at ? `<div style="font-size:12px;color:#64748b">Gültig bis: ${formatDate(e.expires_at)}</div>` : ''}
        ${e.notes      ? `<div style="font-size:12px;color:#64748b;margin-top:6px">${esc(e.notes)}</div>` : ''}
      </div>
    `).join('');
    ...
    wrap.innerHTML = `
      ...
      <p style="font-size:12px;color:#64748b;margin-top:16px">Ausrüstung und Ausweise werden vom Gerätewart oder Wehrleiter gepflegt.</p>
    `;
  ...
  try {
      ...
      wrap.innerHTML = `
        <div class="card">
          <div class="card__body" style="text-align:center;padding:40px 20px">
            ...
            <p style="color:#64748b;font-size:14px">Keine Termine vorhanden.</p>
          </div>
        </div>`;
      renderIcons(wrap);
      return;
    ...
    const renderCards = (list) => list.map(t => {
      const colorDot = t.typ_color
        ...
        : '';
      return `
        <div style="border:1px solid #e2e8f0;border-radius:8px;padding:14px 16px;margin-bottom:10px">
          <div style="display:flex;align-items:flex-start;gap:12px">
            <div style="min-width:52px;text-align:center;background:#f4f5f7;border-radius:6px;padding:6px 8px">
              <div style="font-size:18px;font-weight:700;line-height:1">${new Date(t.start_at).getDate().toString().padStart(2,'0')}</div>
              <div style="font-size:10px;color:#64748b;text-transform:uppercase">${new Date(t.start_at).toLocaleDateString('de-DE', {month:'short'})}</div>
            </div>
            <div style="flex:1">
              ...
              ${t.location ? `<div style="color:#64748b;font-size:12px;margin-top:2px">${icon('map-pin',11)} ${esc(t.location)}</div>` : ''}
    ...
    wrap.innerHTML = `
      ...
      ${past.length ? `
        <div class="card">
          <div class="card__header" style="color:#64748b">Vergangene Termine (${past.length})</div>
