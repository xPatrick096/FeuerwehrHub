  ...
  content.innerHTML = `
    ...
    <div id="announcements-section">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <h3 style="margin:0;font-size:15px;font-weight:600;color:#1e293b">Ankündigungen</h3>
        ${isAdmin ? `<button class="btn btn--primary btn--sm" id="btn-new-announcement">+ Neu</button>` : ''}
      </div>
      <div id="announcements-list"><p style="color:#64748b;font-size:13px">Lade...</p></div>
    </div>

    ${modules.verein ? `
    <div id="schwarzesbrett-section" style="margin-top:2rem">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <h3 style="margin:0;font-size:15px;font-weight:600;color:#1e293b">${icon('clipboard-list', 14)} Schwarzes Brett</h3>
        <a href="#/verein" style="font-size:12px;color:#64748b;text-decoration:none">Verwalten →</a>
      </div>
      <div id="schwarzesbrett-list"><p style="color:#64748b;font-size:13px">Lade...</p></div>
    </div>` : ''}
...
async function loadAnnouncements(user, isAdmin) {
  ...
  try {
    ...
    if (!items.length) {
      list.innerHTML = `<p style="color:#64748b;font-size:13px">Keine Ankündigungen vorhanden.</p>`;
      return;
    }
    ...
    list.innerHTML = items.map(a => `
      <div class="card announcement-card" data-id="${a.id}" style="margin-bottom:12px">
        ...
        <div class="card__body" style="white-space:pre-wrap;font-size:14px;color:#1e293b">
          ${esc(a.content)}
        </div>
      </div>
    `).join('');
...
async function loadPersonalWidget() {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="stat-tiles">
        ${tile(s.active_members,             'Aktive Mitglieder',          '#1e293b')}
...
async function loadIncidentWidget() {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="stat-tiles">
        ${tile(s.total,   `Einsätze ${s.year}`, '#1e293b')}
...
async function loadTermineWidget(modules = {}) {
  ...
  try {
    ...
    if (!upcoming.length) {
      content.innerHTML = `<p style="color:#64748b;font-size:13px;margin:0">Keine bevorstehenden Termine.</p>`;
      return;
    }
    ...
    content.innerHTML = upcoming.map(t => {
      ...
      const colorDot = t._source === 'verein'
        ? `<span style="display:inline-block;padding:1px 5px;border-radius:999px;font-size:9px;font-weight:600;background:#e2e8f0;color:#64748b;margin-right:5px;flex-shrink:0">Verein</span>`
        : t.typ_color
          ...
          : '';
      return `
        <div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid #e2e8f0">
          <div style="min-width:40px;text-align:center;background:#f4f5f7;border-radius:5px;padding:4px 6px;flex-shrink:0">
            <div style="font-size:14px;font-weight:700;line-height:1">${String(d.getDate()).padStart(2,'0')}</div>
            <div style="font-size:9px;color:#64748b;text-transform:uppercase">${d.toLocaleDateString('de-DE',{month:'short'})}</div>
          </div>
...
async function loadSchwarztesBrett(isAdmin) {
  ...
  try {
    ...
    if (!posts?.length) {
      list.innerHTML = `<p style="color:#64748b;font-size:13px">Keine Beiträge vorhanden.</p>`;
      return;
    }
