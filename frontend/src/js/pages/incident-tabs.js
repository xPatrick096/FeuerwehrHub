...
function _renderVehiclesTab(wrap, incidentId, rows, vehicleOptions, readonly) {
  ...
  const tableHTML = rows.length ? `
    ...
    </div>` : `<p style="color:var(--text-muted);font-size:13px;margin-bottom:16px">Noch keine Fahrzeuge eingetragen.</p>`;

  const addFormHTML = readonly ? '' : `
    <div id="v-add-form" style="display:none;background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:12px">
...
function _renderPersonnelTab(wrap, incidentId, rows, memberOptions, readonly) {
  const tableHTML = rows.length ? `
    ...
    </div>` : `<p style="color:var(--text-muted);font-size:13px;margin-bottom:16px">Noch keine Kräfte eingetragen.</p>`;

  const addFormHTML = readonly ? '' : `
    <div id="p-add-form" style="display:none;background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:12px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div class="form-group" style="grid-column:1/-1">
...
function _renderAttachmentsTab(wrap, incidentId, attachments, readonly) {
  ...
  const listHTML = attachments.length ? `
    ...
    </div>` : `<p style="color:var(--text-muted);font-size:13px;margin-bottom:16px">Keine Anhänge vorhanden.</p>`;

  const uploadHTML = readonly ? '' : `
    <div id="attach-drop-zone"
         style="border:2px dashed var(--border);border-radius:8px;padding:24px;text-align:center;cursor:pointer;
                transition:border-color 0.15s">
      <div>${icon('paperclip', 28)}</div>
      <div style="font-size:13px;color:var(--text-muted);margin-top:6px">Datei hierher ziehen oder klicken zum Auswählen</div>
      <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Bilder (JPEG/PNG/GIF/WebP), PDF, Word (docx), ODT, Text — max. 20 MB</div>
      <input type="file" id="attach-file-input" style="display:none"
             accept="image/*,.pdf,.docx,.odt,.txt">
    </div>
    <div id="attach-progress" style="display:none;margin-top:8px;font-size:13px;color:var(--text-muted);text-align:center">
      Wird hochgeladen...
    </div>
  `;
